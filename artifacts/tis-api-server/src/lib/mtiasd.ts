 ...
 *
 * Pure data + small helpers, no PDFKit dependency (mirrors fdot-gsvt.ts) so any
 * renderer can consume it. Self-contained: the MTIASD citation and the per-state
 * controlling impact paradigm (controllingImpactParadigm, MTIASD_CITATION) live
 * here too, so the methodology chapter depends on this module alone.
 *
...
// ─── Citation + per-state controlling impact paradigm ─────────────────────────
