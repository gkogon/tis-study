 ...
 * within-day office *shape* that a renderer distributes the engine's GROSS ITE
 * trip generation across; the *magnitude* is always the engine's own trip
 * generation, never fabricated.
 *
 * Two office profiles exist, selected by `ProfileLocale` so a renderer only
 * ever prints a provenance line with a nexus to its own jurisdiction:
 *
 *  - locale "us" (DEFAULT): the ITE Trip Generation Manual, 11th Edition, Land
 *    Use 710 (General Office Building) weekday time-of-day distribution, with
 *    NCHRP Report 765 as the roadway-diurnal fallback. This is the only office
 *    profile a US DOT renderer (FDOT/GDOT/Caltrans/NYSDOT/TxDOT/IDOT) may emit.
 *
...
/**
 * Jurisdiction selector for the office profile. "us" (default) uses the ITE TGM
 * shape + source string; "uk" uses the LTDS/Velocity shape + source string. Only
 * the London/UK renderer may pass "uk"; every other renderer must use the US
 * default so no TfL/LTDS provenance line reaches a US DOT submittal.
 */
...
// Office shape from the ITE Trip Generation Manual, 11th Edition, Land Use 710
// (General Office Building) weekday time-of-day data: a strong AM ENTERING peak
// at 07:00–09:00 and PM EXITING peak at 16:00–18:00, with a midday lunch
// exchange. Values are relative per-hour magnitudes within each series;
// `normalise` rescales each series to sum to 1, so only the shape matters
// (`distributeDaily` sets the inbound and outbound daily totals equal). This is
// the only office profile a US DOT renderer may print.
const ITE_OFFICE_ARRIVALS_RAW = [
  1, 1, 0, 0, 1, 4, 18, 70, 90, 38, 14, 12,
  16, 14, 9, 7, 5, 4, 3, 2, 1, 1, 1, 1,
];
const ITE_OFFICE_DEPARTURES_RAW = [
  1, 1, 1, 0, 0, 1, 2, 4, 6, 7, 8, 12,
  20, 16, 10, 14, 40, 90, 50, 22, 10, 5, 3, 2,
];

const ITE_OFFICE_SOURCE =
  "Within-day distribution from the ITE Trip Generation Manual, 11th Edition, Land Use 710 " +
  "(General Office Building) weekday time-of-day data — a strong AM entering peak (07:00–09:00) " +
  "and PM exiting peak (16:00–18:00), with a midday lunch exchange. The profile shape is the ITE " +
  "hourly distribution; absolute volumes scale the engine's gross ITE trip generation. Where a " +
  "site-specific hourly distribution is unavailable, NCHRP Report 765 (Analytical Travel Forecasting " +
  "Approaches for Project-Level Planning and Design) time-of-day factors are the roadway-diurnal fallback.";

/** The default (US) office profile: ITE TGM 11th Ed. LU 710 time-of-day shape. */
export const ITE_OFFICE_PROFILE: DiurnalProfile = {
  arrivals: normalise(ITE_OFFICE_ARRIVALS_RAW),
  departures: normalise(ITE_OFFICE_DEPARTURES_RAW),
  source: ITE_OFFICE_SOURCE,
};
...
const LTDS_SOURCE =
  ...
  "absolute volumes scale the engine's gross ITE trip generation.";
...
// --- Non-office profiles (standard US distributions: NHTS 2022 + ITE TGM 11th
// Ed. time-of-day, designed and adversarially shape-verified). Un-normalised;
// `normalise` rescales each series to sum to 1. ----------------------------
...
export const RESIDENTIAL_PROFILE: DiurnalProfile = {
  ...
  source:
    "Within-day distribution from the FHWA National Household Travel Survey (NHTS 2022) home-based trip start times, cross-checked against ITE Trip Generation Manual 11th Ed. LU 210/220 directional splits. Residents depart in the AM peak and return in the PM peak; on-site occupancy is highest overnight and lowest midday.",
};
...
export const RETAIL_PROFILE: DiurnalProfile = {
  ...
  source:
    "Within-day distribution from NHTS 2022 shopping / errand / meal trip start times, cross-checked against ITE Trip Generation Manual 11th Ed. time-of-day data for LU 820 / 850 / 9xx. Activity builds to a midday plateau and a stronger late-afternoon/evening peak; on-site occupancy peaks midday through the evening.",
};
...
export const SCHOOL_PROFILE: DiurnalProfile = {
  ...
  source:
    "Within-day distribution from NHTS 2022 school-trip start times, cross-checked against ITE Trip Generation Manual 11th Ed. time-of-day data for LU 520 / 530. A sharp AM arrival peak before first bell and an early-afternoon dismissal departure peak; on-site population plateaus through the school day and empties at dismissal.",
};

/**
 * Validate + normalise a consultant-supplied override. Returns null when the
 * payload is missing or unusable, so the caller can fall back to the LTDS
...
export function parseProfileOverride(raw: unknown): DiurnalProfile | null {
  ...
  const src =
    typeof o.source === "string" && o.source.trim()
      ? o.source.trim()
      : "Consultant-supplied within-day trip profile (overrides the engine's default office distribution).";
  return { arrivals: normalise(a), departures: normalise(d), source: src };
}

/** The office profile for a locale: ITE TGM (US default) or LTDS/Velocity (UK only). */
export function officeProfile(locale: ProfileLocale = "us"): DiurnalProfile {
  return locale === "uk" ? LTDS_OFFICE_PROFILE : ITE_OFFICE_PROFILE;
}

/** Resolve the active profile: a valid consultant override, else the locale office default. */
export function resolveProfile(override?: unknown, locale: ProfileLocale = "us"): DiurnalProfile {
 ...
 * (standard US NHTS 2022 + ITE TGM distributions). `office` is intentionally
 * absent here because it is locale-gated — resolve it via `officeProfile(locale)`
 ...
 * note instead of fabricating a curve).
 */
...
/** Full profile registry for a locale: the shared families plus the locale's office shape. */
export function profilesForLocale(locale: ProfileLocale = "us"): Partial<Record<UseFamily, DiurnalProfile>> {
  return { ...PROFILES, office: officeProfile(locale) };
}

/**
 * Map an ITE Trip Generation land-use code to a use-class family.
 ...
 * false). `locale` selects the office shape — "us" (default) → ITE TGM, "uk" →
 * LTDS/Velocity — and bounds the unmatched fallback's source so a US caller can
 * never surface a TfL/LTDS string.
 */
export function profileForLandUse(
  ...
  return { profile: officeProfile(locale), matched: false, family };
}
