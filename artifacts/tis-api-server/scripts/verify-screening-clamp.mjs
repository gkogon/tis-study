/**
 * Accuracy/credibility guard: the HCM incremental-delay term (d2) grows without
 * bound above capacity, so at v/c ≈ 2 the raw formula returns ~500–600 s of
 * control delay — physically implausible and indefensible in a screening
 * deliverable (observed live: a Miami-Dade demo showed 583 s / v/c 2.23).
 * vcToDelay must cap reported delay at SCREENING_MAX_DELAY_SEC while preserving
 * LOS F (the cap is above the 80 s LOS-F threshold).
 *
...
ok(SCREENING_MAX_DELAY_SEC === 120, `SCREENING_MAX_DELAY_SEC is 120 (got ${SCREENING_MAX_DELAY_SEC})`);
...
ok(vcToDelay(1.47) === SCREENING_MAX_DELAY_SEC, `v/c 1.47 (was ~244s) clamped to ${SCREENING_MAX_DELAY_SEC}s`);
