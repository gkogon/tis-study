...
| # | Document | Publisher | Edition | URL | Role |
|---|---|---|---|---|---|
| 1.27 | **TRICS** | TRICS Consortium Ltd | Live (quarterly updates) | https://trics.org/ | Subscription multi-modal trip-rate database. UK industry standard; ITE not accepted. |
...
- ~110 land-use sub-categories, ~5,200 sites, >10,000 surveys (UK + Ireland). ~36% of sites carry multi-modal counts.
- **85th-percentile rate is the canonical UK starting point** for TA work (DfT 2007 §4.62), not the mean. Renderer reports both.
- Multi-modal output: car, motorcycle, LGV, HGV, PSV/bus, taxi, walking, cycling.
- Subscription-only — renderer accepts TRICS rate set as user input (CSV or scoped-and-locked TRICS rank/filter parameters), does not query TRICS directly. Recording the TRICS scenario filter (date band, region, day type, parking provision, GFA range) in the methodology section is non-negotiable for reviewer audit.
...
- Inputs:
  - **SAP** = Service Access Point — bus stops within 640 m (8 min walk at 80 m/min) and rail/tube/Overground/DLR/Elizabeth/tram/river-bus stations within 960 m (12 min walk).
  - **EDF** = Equivalent Doorstep Frequency — AM-peak service frequency 08:15–09:15, adjusted for walk time and reliability.
  - **AI** = Accessibility Index — sum of weighted EDFs across all routes, banded into a PTAL grade (AI 0–5 → PTAL 1, ≥40 → PTAL 6b).
- **Required in every London TA.** Drives car-parking maximums under **London Plan policy T6** sub-policies. PTAL 5/6a/6b = "car-free starting point" in well-connected places.
...
**Still open:**
1. **PTAL methodology primary source** — SAP / EDF / AI bandings and the "PTAL 5 / 6a / 6b → car-free starting point" framing were not primary-source verified. Wikipedia and secondary blogs cover it; the authoritative source is TfL's PTAL methodology PDF on londondatastore — confirm before deployment.
2. **TRICS 2026 update** — TRICS posted a revised licence and bureau-fee schedule effective 1 January 2026 (`trics.org/2025/10/02/...`); confirm whether trip-rate methodology or post-COVID baseline inclusion criteria also changed.
