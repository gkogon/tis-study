...
- **`report_template` is not in the per-request firm projection**: `getActiveFirmForUser` returns `FirmLite` (every column but the theme); the PDF render and the `/firms/report-template*` routes fetch it with `loadFirmReportTemplate(firmId)`. A new route that needs the theme must do the same — `firm.reportTemplate` no longer exists on the request's firm.
