...
- **Report theme (firm format import)**: `artifacts/tis-api-server/src/lib/report-theme/` — `extract.ts` (sample PDF → Theme via the pdfjs operator-list scanner `pdf-scan.ts` and `derive/*`), `theme.ts` (schema + DEFAULT_THEME), `active.ts` (per-render module state), `draw.ts` (themed primitives the regional renderers delegate to), `layout.ts` (geometry-aware figure scaling and section breaks), `fonts.ts` (bundled substitute fonts under `data/fonts/<family>/`), `preview-fixtures.ts` (`data/preview-fixtures/*.json`), `derive/figures.ts` (figure caption convention). Library: `src/lib/report-themes.ts` (rows in `firm_report_themes`, firm default `firms.default_report_theme_id`, per-project `tis_projects.report_theme_id`, `resolveProjectTheme` = the project's theme → firm default → standard). Routes: `GET/POST /tis-api/firms/report-themes`, `PATCH/DELETE /tis-api/firms/report-themes/:id`, `GET …/:id/preview.pdf`, `PATCH /tis-api/projects/:id { reportThemeId }`; `/generate` and `/generate/pdf` take `?reportThemeId=` or `reportThemeId` in the body. Frontend: `components/report-formats-card.tsx` (settings), `components/format-setup-card.tsx` (first-run prompt), format pickers on the study and project pages. Checks: `pnpm --filter @workspace/tis-api-server run check:theme`. Specs: `docs/superpowers/specs/2026-09-14-report-theme-import-design.md`, `2026-09-16-report-theme-library-design.md`.
...
- **Default theme is byte-pinned**: `check:theme-default-identity` compares all seven default renders (TX/NC/SC offline; FL/GA/NY and the UK template path with `fetch` stubbed to fail, pinning each renderer's no-network fallback) against `scripts/fixtures/theme-identity-baseline.json`. Re-pin (`--pin`) only for a deliberate render change, in the same PR that makes it.
- **Themes live in `firm_report_themes`, never on the request's firm**: `getActiveFirmForUser` returns `FirmLite` (no `reportTemplate`); every render resolves its theme with `resolveProjectTheme(firmId, project.reportThemeId)` from `lib/report-themes.ts`. A stale or foreign theme id is ignored with a warn (the render falls back), never refused. `firms.report_template` now only ever holds a V1 legacy row; the V2 backfill into the library ran once in `lib/db/migrate.mjs`. Deleting a theme clears the firm default and pinned projects explicitly — the SET NULL foreign keys exist only on databases migrated by `migrate.mjs`, not on ones pushed straight from the drizzle schema.
- **Figure captions follow the sample**: `theme.figure.{label,numbering,separator}` (optional; absent → our "Figure — Title"). Under a theme every chart, the compass rose and the distribution plan caption through `themed.figureCaption`, numbered per render in `active.ts` (chapter numbering restarts at each level-1 heading). Table and figure captions both vote on the convention in `derive/figures.ts`; a firm with thirty numbered tables and two figures still gets its separator. Not modelled: table numbering (our tables have no captions) and full-page exhibit layout.
- **Photo covers**: a full-width picture on the sample's cover becomes a band with `photo: true`; the renderer fills it with the study's own Street View site photo and falls back to the band's mean colour (the sample's picture is another project's site and is never reproduced).
...
- **Renderer layout is tuned to the default text box — themed renders must be geometry-aware**: fixed figure heights (200 pt chart plots, the 330 pt distribution plan, 132 pt turning-movement diagrams), unconditional `doc.addPage()` before a major section, and one-worksheet-per-page all assume 512 × 692 pt at 10 pt. A firm theme's smaller box turned those into half-empty and two-line pages. Use `report-theme/layout.ts`: `scaledHeight(doc, h)` for any fixed figure height, `sectionBreak(doc)` instead of a bare `addPage()` before a section, `keepTogether` / `heading(..., { keepWith })` for a heading over a table or figure. All are the identity under the default theme. `check:theme-render` gates page fill (no near-empty page, none under 35 % full).
