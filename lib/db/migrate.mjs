...
const SQL_STATEMENTS = [
  ...
  // Trip-generation flywheel — one append-only row per study capturing
  // inputs + computed trips + rate provenance. The accumulating source for
  // an owned, ITE-independent trip-rate dataset (re-derive/calibrate rates
  // from real usage once it builds). Fire-and-forget writes; never read on
  // the hot path. See lib/db/src/schema/trip-gen-samples.ts.
  {
    id: "trip_gen_samples.create",
    sql: `CREATE TABLE IF NOT EXISTS trip_gen_samples (
      id BIGSERIAL PRIMARY KEY,
      land_use_code VARCHAR(16) NOT NULL,
      land_use_name VARCHAR(128),
      unit VARCHAR(32),
      size DOUBLE PRECISION,
      latitude DOUBLE PRECISION,
      longitude DOUBLE PRECISION,
      region_code VARCHAR(48),
      daily_trips INTEGER,
      am_trips INTEGER,
      pm_trips INTEGER,
      rate_confidence VARCHAR(32),
      rate_source VARCHAR(512),
      observed_daily_trips INTEGER,
      observed_source VARCHAR(128),
      firm_id VARCHAR,
      user_id VARCHAR,
      metadata JSONB,
  ...
  { id: "trip_gen_samples.idx_landuse", sql: `CREATE INDEX IF NOT EXISTS "IDX_tgs_landuse_created" ON trip_gen_samples (land_use_code, created_at);` },
  { id: "trip_gen_samples.idx_region",  sql: `CREATE INDEX IF NOT EXISTS "IDX_tgs_region" ON trip_gen_samples (region_code);` },
  { id: "trip_gen_samples.idx_created", sql: `CREATE INDEX IF NOT EXISTS "IDX_tgs_created" ON trip_gen_samples (created_at);` },
];
