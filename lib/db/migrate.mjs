...
const SQL_STATEMENTS = [
  ...
  // Firm cover-branding fields (drive the white-labeled PDF cover).
  { id: "firms.brand_color",  sql: `ALTER TABLE firms ADD COLUMN IF NOT EXISTS brand_color VARCHAR(9);` },
  { id: "firms.address_line", sql: `ALTER TABLE firms ADD COLUMN IF NOT EXISTS address_line TEXT;` },
  { id: "firms.phone",        sql: `ALTER TABLE firms ADD COLUMN IF NOT EXISTS phone VARCHAR(40);` },
  { id: "firms.website",      sql: `ALTER TABLE firms ADD COLUMN IF NOT EXISTS website VARCHAR(255);` },

  // Stripe webhook idempotency ledger — dedup replayed / double-delivered
  // Stripe events so a re-delivered invoice.paid can't re-grant a billing
  // period's study quota. See lib/db/src/schema/tis-billing.ts.
  {
    id: "stripe_webhook_events.create",
  ...
  // Pay-per-study credits. study_credits_remaining = banked prepaid studies
  // (permanent, consumed after period quota is exhausted). studies_purchased_
  // lifetime = monotonic counter that selects the intro vs standard à-la-carte
  // rate at checkout. See lib/db/src/schema/firms.ts + lib/billing.ts.
  { id: "firms.study_credits_remaining",    sql: `ALTER TABLE firms ADD COLUMN IF NOT EXISTS study_credits_remaining INTEGER NOT NULL DEFAULT 0;` },
  { id: "firms.studies_purchased_lifetime", sql: `ALTER TABLE firms ADD COLUMN IF NOT EXISTS studies_purchased_lifetime INTEGER NOT NULL DEFAULT 0;` },

  // The firm's own report format, ingested from an example PDF (POST
  // /firms/report-template). Previously written only to the container
  // filesystem by report-template/store.ts, which on Railway is wiped by every
  // deploy — so an uploaded template survived until the next push and then
  // silently reverted to the region default. See lib/db/src/schema/firms.ts.
  { id: "firms.report_template",            sql: `ALTER TABLE firms ADD COLUMN IF NOT EXISTS report_template JSONB;` },
  // Report theme library (docs/superpowers/specs/2026-09-16-report-theme-library-design.md):
  // a firm keeps several formats, has a default, and a project may pin one.
  ...
  // Backfill: a firm's single imported format (a V2 row still in
  // report_template) becomes its first, default theme; the column is then
  // cleared so only V1 ("legacy") rows remain there.
  ...
  { id: "firms.default_report_theme_id.backfill", sql: `UPDATE firms f SET default_report_theme_id = t.id, report_template = NULL
      ...
      WHERE t.firm_id = f.id AND f.default_report_theme_id IS NULL
        AND f.report_template IS NOT NULL AND f.report_template->>'version' = '2';` },
];
